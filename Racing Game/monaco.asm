...
;
; Draws a single-colored 8px by 8px car 
; User can recolor and move car based on
; keyboard input. Car rotates to match
; current saved direction.
;_________________________________________
;
...
Init	AND R0, R0, 0	;\
	...
	ST R1, CurCol	; Resets current color value
	LD R0, CarInit	
	ST R0, CarPtr	; Load and store car pointer
	ADD R6, R6, 1	; Set column offset to 1
	JSR PaintH	; Add car to screen
...
LMove	LD R1, BLACK	; Load value for black into R1
	JSR Paint	; Clear car
	LD R1, CarPtr	; Load current car location
	ADD R1, R1, -8	; and offset 8 pixels left
	ST R1, CarPtr	; Update car pointer
	ADD R0, R1, 0	; Load car pointer
	JSR PaintH	; and restore car color
	JSR ColDet	; Check for pylon collision
...
DMove	LD R1, BLACK	; Load value for black into R1
	...
	ST R1, CarPtr	; Update car pointer
	LD R1, CurCol	; Load preexisting car color
	JSR Paint	; and restore car color
	JSR ColDet	; Check for pylon collision
	JSR AddFns	; Update Environment
	BRnzp Input
...
RMove	LD R1, BLACK	; Load value for black into R1
	...
	ST R1, CarPtr	; Update car pointer
	ADD R0, R1, 7	; Load printing car pointer
	AND R0, R0, 0	
	ADD R0, R0, -1	; Set column offset to -1
	JSR PaintH	; and restore block color
	JSR ColDet	; Check for pylon collision
	JSR AddFns	; Update Environment
	AND R0, R0, 0
	ADD R0, R0, 1
	ST R0, DirSave	; Set saved car orientation to right
	BRnzp Input
...
; Paint:
; ------
; Subroutine to clear car from screen
...
; PaintH:
; ------
; Subroutine to paint left or right
; moving car onto screen.
; Modified CarPtr should be listed
; in R0 with Column offset in R6: 
; (-1 or 1)
;------------------------------------
...
PaintH	AND R4, R4, 0	
	ADD R4, R4, 8	; Set "rows to color" loop counter to 8
	LD  R3, LineDwn	; Loads row length
	LD  R5, H4000	; Loads car pixel color pointer
	AND R2, R2, 0	
Hlp1	ADD R2, R2, 8	; Set "pixels to color" loop counter to 8
	ST  R0, TempPt	; Pointer to current leftmost pixel is stored
Hlp2    LDR R1, R5, 0	; Color of car pixel is moved into R1
	STR R1, R0, 0 	; Video pixel is colored
	ADD R5, R5, 1	; Pixel color pointer is incremented
	ADD R0, R0, R6	; Advance video memory pointer 1 column
	ADD R2, R2, -1	; Amount of pixels to color is decremented
	BRp Hlp2	; Checks if pixels are left to be colored		
	LD  R0, TempPt	; Loads the leftmost pixel pointer
	ADD R0, R0, R3	; Moves the saved video pointer one pixel down
	ADD R4, R4, -1	; Amount of rows to color is decremented
	BRp Hlp1 	; Checks if rows are left to be colored		
	RET

;------------------------------------
; PaintV:
; ------
; Subroutine to paint up or down
; moving car onto screen.
; Modified CarPtr should be listed
; in R0 with Column offset in R6: 
; (-1 or 1), Row offset in R3: 
; (-x80 or x80) and pixel memory pointer
; in R5
;------------------------------------

PaintV	ST R7, retSave
	AND R4, R4, 0
	ADD R4, R4, 8
	LD R7, LineDwn
	AND R2, R2, 0
...
TempMem	.FILL	x0000	; Temporary memory for coloring pointer
DirSave	.FILL	xFFFF	; Car orientation storage, initialized to left
retSave	.FILL	x0000	; Storage for R7
H4000	.FILL 	x4000	; Location of 8x8 pixel scheme array
H4007	.FILL	x4007	; Literal value, upper right pixel in car icon
H4038	.FILL	x4038	; Literal value, lower left pixel in car icon
